...
export default function AddSongPage() {
    ...
    const handleToggleChords = () => {
        ...
        // In AddSongPage, the editor container might be at a different index depending on the tabs,
        // but typically it's the 3rd child (header, title inputs, sticky toolbar, editor). Let's find it safely.
        // For safety, we can skip locking height or try to find it by class.
        const editorContainer = el.querySelector('.editor-container') as HTMLElement;
        if (editorContainer) {
    ...
    async function submit() {
        if (!title.trim() || !artist.trim()) {
            showToast('Назва та виконавець обов\'язкові');
            return;
        }
        ...
        try {
            ...
            showToast("Пісня успішно додана! Повертаємось на головну...");
            router.push('/');
        } catch (e) {
            ...
            <div className="mx-auto flex h-full w-full max-w-3xl flex-col border-x-2 border-black/5 dark:border-white/5 relative">
                ...
                <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide flex flex-col">
                    <div className="px-4 flex flex-col gap-2 shrink-0 mb-4 pt-4">
                        ...
                        <div className={`flex flex-col gap-2 shrink-0 transition-opacity ${activeTab !== 'manual' ? 'opacity-50 pointer-events-none' : ''}`}>
                            <label className="block">
                    ...
                    <div className={`sticky top-0 z-10 flex flex-col w-full shadow-sm transition-opacity ${activeTab !== 'manual' ? 'opacity-50 pointer-events-none' : ''}`}>
                    ...
                    <div className={`flex flex-col flex-1 min-h-[calc(100dvh-150px)] px-4 pt-4 pb-4 transition-opacity editor-container ${activeTab !== 'manual' ? 'opacity-50 pointer-events-none' : ''}`}>
                        <div className="flex-1">
                            {showChords ? (
                                ...
                                <textarea
                                    ...
                                    className={`w-full resize-none rounded border-2 px-2 py-2 text-center outline-none font-sans font-black leading-[1.8] whitespace-pre-wrap break-words overflow-hidden transition ${
                                        isEditingText 
